...
function Needs() {
	...
	const selectOptions = [
	  { value: EXPAND_VALUE, label: "Expand all" },
	  { value: COLLAPSE_VALUE, label: "Collapse all" }
	];
		...
		if (familyRecId){
			return (
				<div>
					...
					<SelectButtons
						  value={selectValue}
						  onChange={newValue => selectChange(newValue, setSelectValue, wrapped, setWrapped)}
    ...
    return (
        <Row hasThickBorderBottom={true}>
			<Cell width={EXPAND_CELL_WIDTH_PERCENTAGE}>
				<Text textColor="light">Expand</Text>
			</Cell>
		...
		return (
			<Row>
				<Cell width={EXPAND_CELL_WIDTH_PERCENTAGE}>
					<input
						type='button'
						onClick={() => expand(record, wrapper, setter)}
						value={buttonValue}
					/>
				</Cell>
				<Cell width={ITEM_CELL_WIDTH_PERCENTAGE}>
					<Text fontWeight="strong">{record.getType()}</Text>
				</Cell>
				<Cell width={CATEGORY_CELL_WIDTH_PERCENTAGE}>
...
function familySelected(setFamilyRecId, recordid, templateRecords, setWrapped, templateNameField, templateTypeField){
	...
	for (let record of templateRecords){
		...
		if (cat != record.getCellValueAsString(templateTypeField)){
...
// see comments for increment above
// if its a catagory header then toggle the expand status and replicate
// that to the rest of the rows with that catagory
function expand(record, wrapper, setWrapper){
	if (record.getHeader()){  // only the catagory header rows have the expand button,
							  // but play safe
		record.toggleExpanded();
		let value = record.getExpanded();
		let currentCatagory = record.getType();
		let cat = "";
		for (let r of wrapper){
			
			if (!r.getHeader() && r.getType() == currentCatagory){
				r.setExpanded(value);
			}
			cat = r.getType();
		}
...
function selectChange(newValue, settera, wrapper, setterb){
	let expanded = true;
	if (newValue == EXPAND_VALUE){
		expanded = true;
	} else {
		expanded = false;
	}
